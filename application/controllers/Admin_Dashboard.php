...
=======
        ...
        $data['account'] = $this->CommonModal->getRowByIdDesc('account', 'user_id', $uid[0]['id'], 'id', 'DESC');
        if ($this->input->post()) {
>>>>>>> 07af657fc997b0a76e3a62e539fc93560f3b210a
            $post = $this->input->post();
...
>>>>>>> 07af657fc997b0a76e3a62e539fc93560f3b210a
    ...
    public function add_invoice($id)
    {
        ...
        if ($_POST) {
            ...
            $customer = $this->CommonModal->getRowById('customer', 'id', $c_names);
            $interest_rate = !empty($customer) ? floatval($customer[0]['interest_rate']) : 0;
            $interest_days = !empty($customer) ? intval($customer[0]['interest_days']) : 0;
            ...
            if ($current_date >= $due_date) {
                $days_late = ceil(($current_date - $due_date) / (60 * 60 * 24));
                $daily_interest = ($final * ($interest_rate / 100)) / 365;
                $interest_amount = $daily_interest * $days_late;
            } else {
                $days_late = 0;
                $interest_amount = 0;
            ...
            foreach ($product_names as $index => $product_name) {
                ...
                if ($include_interest == '1') {
                    $data_to_insert['intrest_rate'] = $interest_rate;
                    $data_to_insert['intrest_days'] = $interest_days;

                    $data_to_insert['late_days'] = $days_late;
                    $data_to_insert['last_date'] = date('Y-m-d', $due_date);
                    $data_to_insert['total_with_intrest'] = $final + $interest_amount;
                }
                ...
                if ($savedata) {
                    ...
                    if ($box[$index] == "Box") {
                        ...
                        $new_per = $new_available_quantity * $per_box[$index];


                        $updatedata1 = $this->CommonModal->updateColumnValue(
                            'purchase_product',
                            'p_id',
                            $product_ids[$index],
