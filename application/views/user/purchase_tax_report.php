                        ...
                        <div class="filter cm-content-box box-primary relative rounded mb-4 bg-white dark:bg-[#182237]">
                            ...
                            <div class="content form excerpt border-t border-b-color dark:border-[#ffffff1a]">
                                <form class="profile-form"
                                    action="<?= base_url('admin_Dashboard/purchase_tax_report/' . encryptId($user['0']['id'])) ?>"
                                    ...
                                    <div class="sm:p-5 p-4">
										...
										<div class="sm:w-1/4 w-full mb-[30px]">
                                            ...
                                            <div class="xl:w-1/4 sm:w-1/2 w-full self-end">
                                                ...
                                                <a
                                                    href="<?= base_url('Admin_Dashboard/purchase_tax_report/' . encryptId($user['0']['id'])) ?>"><button
                ...
                <div class="xl:w-1 col-xxl-12">
						<div class="card">
							...
							<div class="sm:pt-5 pt-4 flex-auto dz-tab-area">
								<ul class="nav nav-tabs nav-pills success-tab px-5 mb-5 flex flex-wrap" id="pills-tab" role="tablist">
								  ...
								  <li class="nav-item" role="presentation">
									<button class="nav-link rounded-md px-[15px] py-[5px] text-[13px] active block tab-btn" data-tab="pills-sale" type="button" role="tab"  aria-selected="false">
										...
										<span class="dark-2">Purchase</span></button>
								  </li>
								  ...
								  <li class="nav-item ml-[15px]" role="presentation">
									<button class="nav-link rounded-md px-[15px] py-[5px] text-[13px] block tab-btn" data-tab="pills-all1" type="button" role="tab" aria-selected="false">
										...
										<span class="dark-2">Return <br>Purchase</span>
									</button>
								  </li>
								</ul>
								...
								<h4 class="text-base">Purchase Tax Report</h4>
...
PDF
   ...
   <thead>
        <tr>
            ...
            <th class="py-[0.7375rem] p-2.5 whitespace-nowrap uppercase text-[13px] font-medium text-primary border-b border-b-color text-left">Purchase Date</th>
            ...
            <th class="py-[0.7375rem] p-2.5 whitespace-nowrap uppercase text-[13px] font-medium text-primary border-b border-b-color text-left">GST NO.</th>
    ...
    <tbody>
        <?php 
            ...
            foreach ($purchases as $product): 
                ...
                $taxamount = $this->CommonModal->getRowByIdSum('purchase_product', 'purchase_code',  $product['purchase_code'] ,'user_id',$user['0']['id'],'gst_tax');
                $tax = $this->CommonModal->getRowByIdSum('purchase_product', 'purchase_code',  $product['purchase_code'] ,'user_id',$user['0']['id'],'gst_percent');


                $totaltax += $tax[0]['total_sum'];
                $totaltaxamount += $taxamount[0]['total_sum'] ;
        ?>
        <tr>
            ...
            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $vender[0]['gst_no'] ?></td>

            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $product['p_name_count'] ?></td>
            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $tax[0]['total_sum'] ?> %</td>

<td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]">₹ <?= $taxamount[0]['total_sum'] ?></td>
        ...
        <tr>
            ...
            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap border-t border-t-color"></td>
                                 ...
                                 <div class="card-header flex justify-between sm:pt-0 pb-0 py-5 sm:px-5 px-4 items-center relative flex-wrap">
								<h4 class="text-base">Return Purchase Tax Report</h4>
...
PDF
    ...
    <tbody>
        <?php 
            ...
            foreach ($purchases_r as $product): 
                $totalAmount1 += $product['grand_total'];
                $place = $this->CommonModal->getRowByMultitpleId('stock_place', 'id', $product['stock_place_name'], 'user_id', $user[0]['id']);
                $vender = $this->CommonModal->getRowByMultitpleId('vender', 'id', $product['vender_name'], 'user_id', $user[0]['id']);
                $taxamount = $this->CommonModal->getRowByIdSum('return_purchase', 'return_code',  $product['return_code'] ,'user_id',$user['0']['id'],'gst_tax');
                $tax = $this->CommonModal->getRowByIdSum('return_purchase', 'return_code',  $product['return_code'] ,'user_id',$user['0']['id'],'gst_percent');


                $totaltax1 += $tax[0]['total_sum'];
                $totaltaxamount1 += $taxamount[0]['total_sum'] ;
        ?>
        <tr>
            <td class="sm:pl-[1.275rem] pl-[0.9375rem] py-[0.7375rem] pr-[0.625rem] whitespace-nowrap"><?= $serial++ ?></td>
            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $place[0]['place_name'] ?></td>
        ...
        <tr>
            ...
            <td class="py-[0.7375rem] p-2.5 whitespace-nowrap border-t border-t-color">₹ <?= $totaltaxamount1 ?>/-</td>
...
function exportTableAsPDF2() {
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('purchase_tax_report.pdf');
    }).catch((error) => {
...
function exportTableAsPDF3() {
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('return_purchase_tax_report.pdf');
    }).catch((error) => {
        console.error("Error generating PDF:", error);
    });
}
...
function exportTableToExcel3() {
    ...
    // Generate Excel file and download
    XLSX.writeFile(workbook, "return_purchase_tax_report.xlsx");
}
