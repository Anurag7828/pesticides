				...
				<div class="row">
					<div class="w-full">
						<div class="filter cm-content-box box-primary relative rounded mb-4 bg-white dark:bg-[#182237]">
                            ...
                            <form class="profile-form" action="<?= base_url('admin_Dashboard/payment_report/'.encryptId($user['0']['id'])) ?>" method="post" enctype="multipart/form-data">
								<div class="sm:p-5 p-4">
									<div class="row">
											<div class="sm:w-1/3 w-full mb-[30px]">
											<label class="text-dark dark:text-white text-[13px] mb-2">Select Payment Mode</label>
											<select name="mode" class=" choices form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full">
											 <option value="">--SELECT--</option>
                                                <option value="Cash" <?= ($modee == 'Cash') ? 'selected' : ''; ?>>CASH</option>
                                                <option value="Upi"<?= ($modee == 'Upi') ? 'selected' : ''; ?>>UPI</option>
                                               <option value="Card"<?= ($modee == 'Card') ? 'selected' : ''; ?>>CREADIT/DEBIT CARD</option>
                                                                                  
                                               <option value="Bank"<?= ($modee == 'Bank') ? 'selected' : ''; ?>>Bank Transfer</option>
                                             <option value="Cheque"<?= ($modee == 'Cheque') ? 'selected' : ''; ?>>Cheque</option>
											</select>
                                    ...
                                    <div class="sm:w-1/3 w-full ">
										...
										<div class="xl:w-1/3 sm:w-1/2 w-full self-end">
        ...
        <table class="table mb-4 w-full card-table border-no success-tbl" id="u-tbl">
            <thead>
                <tr>
                    ...
                    <th class="py-[0.7375rem] p-2.5 whitespace-nowrap uppercase text-[13px] font-medium text-primary border-b border-b-color text-left">Mode</th>
                </tr>
            </thead>
            <tbody>
                <?php 
                ...
                ?>
                    <tr>
                        ...
                        <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]">₹<?= $product['paid'] ?></td>
                           <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]">₹<?= number_format((float)$product['due'], 2) ?></td>
                        <?php if($product['mode']=="Bank"){
                             $bank = $this->CommonModal->getRowByMultitpleId('account', 'id', $product['bank'], 'user_id', $user['0']['id']); ?>
                                                     <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $product['mode'] ?>-
                        <?= $bank[0]['bank_name'] ?><?= $bank[0]['account_no'] ?></td>
                      <?php  } elseif($product['mode']=="Cheque"){?>
                      
                        <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $product['mode'] ?>-
                        <?= $product['cheque_no'] ?></td>
                        <?php } else{ ?>
                        <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $product['mode'] ?></td>
                        <?php } ?>
...
<a href="#" onclick="exportTableAsPDF1()" class="py-[5px] px-3 text-[13px] rounded text-white bg-primary leading-[18px] inline-block border border-primary duration-500 hover:bg-hover-primary hover:border-hover-primary ml-2">
PDF
</a>
                 </div>
                 </div>
    ...
    <tbody>
        <?php 
            ...
            <tr> 
                ...
                <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $vender[0]['vender_name'] ?></td>
                  <td class="py-[0.7375rem] p-2.5 whitespace-nowrap"><?= $product['date'] ?></td>
                <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]">₹ <?= $product['total'] ?></td>
              
                <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]">₹ <?= $product['paid'] ?></td>
                        <td class="py-[0.7375rem] p-2.5 whitespace-nowrap">₹ <?= number_format((float)$product['due'], 2) ?></td>
                 <?php if($product['mode']=="Bank"){
                             $bank = $this->CommonModal->getRowByMultitpleId('account', 'id', $product['bank'], 'user_id', $user['0']['id']); ?>
                                                     <td class="py-[0.7375rem] p-2.5 whitespace-nowrap text-body-color dark:text-white text-[13px]"><?= $product['mode'] ?>-
                        <?= $bank[0]['bank_name'] ?><?= $bank[0]['account_no'] ?></td>
...
?>
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('purchase_payment_report.pdf');
    }).catch((error) => {
...
function exportTableToExcel() {
    ...
    XLSX.writeFile(workbook, "purchase_payment_report.xlsx");
}
function exportTableAsPDF1() {
    // Check if jsPDF is available
    const { jsPDF } = window.jspdf;
    if (!jsPDF) {
        console.error("jsPDF not loaded");
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('return_purchase_payment_report.pdf');
    }).catch((error) => {
        console.error("Error generating PDF:", error);
    });
}
...
function exportTableToExcel1() {
    ...
    // Generate Excel file and download
    XLSX.writeFile(workbook, "return_purchase_payment_report.xlsx");
}
function exportTableAsPDF2() {
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('sales_payment_report.pdf');
    }).catch((error) => {
        console.error("Error generating PDF:", error);
    });
}
...
function exportTableToExcel2() {
    ...
    // Generate Excel file and download
    XLSX.writeFile(workbook, "sales_payment_report.xlsx");
}
function exportTableAsPDF3() {
    ...
    html2canvas(table, { scale: 2 }).then((canvas) => {
        ...
        // Download the PDF
        pdf.save('return_sales_payment_report.pdf');
    }).catch((error) => {
        console.error("Error generating PDF:", error);
    });
}
...
function exportTableToExcel3() {
    ...
    // Generate Excel file and download
    XLSX.writeFile(workbook, "return_sales_payment_report.xlsx");
}
