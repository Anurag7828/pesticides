                ...
                <div class="row">
                    <div class="w-full">
                        <div class="card mt-4 flex flex-col">
                            <div class="card-header flex justify-between sm:p-5 sm:pt-6 py-5 px-4 items-center relative flex-wrap border-b border-[#E6E6E6] dark:border-[#ffffff1a] sm:text-sm text-xs text-body-color"><span class="float-end"> <strong>Return Code.  </strong> <?= $user['0']['returne_code'] ?>-<?= $purchase_product[0]['return_code']?></span><strong>श्री गणेशाय नमः</strong> <span class="float-end">
                            ...
                            <div class="sm:p-5 p-4 flex-auto">
                                ...
                                <div class="mt-6 lg:w-1/4 md:w-1/2 w-full detail">
									...
									<div class="mt-6 lg:w-1/3 md:w-1/2 w-full detail">
                                        ...
                                        <?php
                            $vender = $this->CommonModal->getRowById('vender', 'id', $purchase_product['0']['vender_name'],'user_id',$user['0']['id']);
                    ...
                    ?>
                                ...
                                <div class="overflow-x-auto table-scroll">
                                    <table class="table w-full mb-4 table-border">
                                        ...
                                        <tbody>
                                        <?php
                            $purchase_code = $this->CommonModal->getRowByMultitpleId('return_purchase', 'return_code', $purchase_product['0']['return_code'],'user_id',$user['0']['id']);
                            $i=0;
                               ...
                               <td class="py-[0.9375rem] px-2.5 border-b border-[#E6E6E6] dark:border-[#ffffff1a] whitespace-nowrap sm:text-sm text-xs text-body-color text-right">₹<?= $row['grand_total']?> /-</td>
                                ...
                                <div class="row">
                                    ...
                                    <div class="lg:w-1/3 sm:w-5/12 w-full ml-auto">
                                        <table class="table w-full mb-4 table-clear">
                                        <?php
                              $payment = $this->CommonModal->getRowByIdOrderByLimit('return_purchase_payment', 'return_code',  $purchase_product['0']['return_code'],'user_id',$user['0']['id'],'id','DESC','1');
                                                          $paymentsum = $this->CommonModal->getRowByIdSum('return_purchase_payment', 'return_code', $purchase_product['0']['return_code'],'user_id',$user['0']['id'],'paid');?>
                                            <tbody>
...
?>
 ...
 document.getElementById('saveButton').addEventListener('click', function () {
            alert("Invoice saved");
              window.location.href = "<?php echo base_url('Admin_Dashboard/return/' . encryptId($user['0']['id'])); ?>";
        });
...
document.getElementById('printBtn').addEventListener('click', function() {
 ...
 document.getElementById('invoicen').addEventListener('click', function() {
            ...
            window.location.href = "<?= base_url('Admin_Dashboard/print_return_purchase_tax/' . encryptId($user['0']['id']) . '/' . $purchase_product[0]['return_code']); ?>";
        });
...
document.getElementById('invoicen1').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Admin_Dashboard/normal_return_purchase/' . encryptId($user['0']['id']) . '/' . $purchase_product[0]['return_code']); ?>";
        });
...
$customer = $this->CommonModal->getRowById('vender', 'id', $purchase_product['0']['vender_name'], 'user_id', $user['0']['id']);

if (!empty($customer)) {  
    foreach ($customer as $cus) {
        // Ensure Return Code is Fetched Properly
        $returnCode = isset($purchase_product['0']['return_code']) ? $purchase_product['0']['return_code'] : 'N/A';

        // Return Purchase Invoice URL
        $returnPurchaseInvoiceUrl = base_url('Admin_Dashboard/print_return_purchase/' . encryptId($user[0]['id']) . '/' . $returnCode);
?>
<script>
    document.getElementById('whatsappBtn').addEventListener('click', function () {
        ...
        let returnPurchaseInvoiceUrl = "<?= $returnPurchaseInvoiceUrl ?>";

        // WhatsApp Message Format
        let message = `*Return Purchase Bill Details*%0A
        *Vendor Name:* ${customerName}%0A
        *Return Code:* ${returnCode}%0A
