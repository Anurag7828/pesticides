...
if (!empty($customer)) {  
    foreach ($customer as $cus) {
        ...
        $returnPurchaseInvoiceUrl = base_url('Admin_Dashboard/normal_return_purchase/' . encryptId($user[0]['id']) . '/' . $returnCode);
?>
<script>
...
document.getElementById('invoicen1').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Admin_Dashboard/print_return_purchase/' . encryptId($user['0']['id']) . '/' . $purchase_product[0]['return_code']); ?>";
        });
