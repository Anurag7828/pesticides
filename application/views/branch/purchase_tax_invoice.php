...
<?php 
    ...
    if (!empty($customer)) {  
        foreach ($customer as $cus) {
            ...
            $invoiceUrl = base_url('Branch_Dashboard/print_purchase_tax/' . encryptId($user[0]['id']) . '/' . $purchase_product[0]['purchase_code']);
?>
<script>
...
?>
 ...
 document.getElementById('invoicen').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Branch_Dashboard/print_purchase/' . encryptId($user['0']['id']) . '/' . $purchase_product[0]['purchase_code']); ?>";
        });
