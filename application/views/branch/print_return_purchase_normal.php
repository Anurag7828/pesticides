...
<button  id="invoicen1" class="py-2 px-4 bg-primary text-white rounded hover:bg-hover-primary duration-300">Normal Invoice</button>
                ...
                <div class="row">
                    <div class="w-full">
                        <div class="card mt-4 flex flex-col">
                            <div class="card-header flex justify-between sm:p-5 sm:pt-6 py-5 px-4 items-center relative flex-wrap border-b border-[#E6E6E6] dark:border-[#ffffff1a] sm:text-sm text-xs text-body-color"><span class="float-end"> <strong>Return Code.  </strong> <?= $user_main['0']['returne_code'] ?>-<?= $purchase_product[0]['return_code']?></span><strong>श्री गणेशाय नमः</strong> <span class="float-end">
            ...
            <div class="sm:w-3/4">
                ...
                <?php } else { ?>
                    <div class="text-body-color sm:text-sm text-xs break-words">
                        <strong style="font-size: 36px; line-height: 47px;"><?= $user_main['0']['shop'] ?></strong>
                    </div>
                <?php } ?>
            </div>
        </div>
    ...
    <div class="mt-6 lg:w-1/3 md:w-1/2 w-full detail text-right">
        ...
        ?>
        <div class="text-body-color sm:text-sm text-xs text-left"><strong>Vender Name:</strong> <?= $vender[0]['vender_name'] ?></div>
        <input type="hidden" id="custommer" value="<?= $vender[0]['vender_name'] ?>">
        <div class="text-body-color sm:text-sm text-xs text-left"><strong>Contact No.:</strong> <?= $vender[0]['mobile'] ?></div>
        <div class="text-body-color sm:text-sm text-xs break-words text-left"><strong>Address:</strong> <?= $vender[0]['address'] ?></div>
        <div class="text-body-color sm:text-sm text-xs text-left"><strong>GST No.:</strong> <?= $vender[0]['gst_no'] ?></div>
    </div>
</div>
    ...
    <?php } ?> /-
                           ...
                           <tr >
                               ...
                               <td class="py-[0.9375rem] px-2.5 border-b border-[#E6E6E6] dark:border-[#ffffff1a] whitespace-nowrap sm:text-sm text-xs text-body-color left strong"><?= convertNumberToWords($row['grand_total'])?></td>
                               <td class="py-[0.9375rem] px-2.5 border-b border-[#E6E6E6] dark:border-[#ffffff1a] whitespace-nowrap sm:text-sm text-xs text-body-color left"></td>
                              <td class="py-[0.9375rem] px-2.5 border-b border-[#E6E6E6] dark:border-[#ffffff1a] whitespace-nowrap sm:text-sm text-xs text-body-color left strong"></td>
 ...
 $paymentsum = $this->CommonModal->getRowByIdSum('return_purchase_payment', 'return_code', $purchase_product['0']['return_code'],'branch_id',$user['0']['id'],'paid');
    ...
    if (!empty($customer)) {  
        foreach ($customer as $cus) {
            ...
            $invoiceUrl = base_url('Branch_Dashboard/print_return_purchase_normal/' . encryptId($user[0]['id']) . '/' . $purchase_product[0]['return_code']);
?>
<script>
...
?>
 ...
 document.getElementById('saveButton').addEventListener('click', function () {
            alert("Invoice saved!");
              window.location.href = "<?php echo base_url('Branch_Dashboard/return/' . encryptId($user['0']['id'])); ?>";
        });
...
document.getElementById('printBtn').addEventListener('click', function() {
 ...
 document.getElementById('invoicen').addEventListener('click', function() {
           ...
           window.location.href = "<?= base_url('Branch_Dashboard/print_return_purchase_tax/' . encryptId($user['0']['id']) . '/' .$purchase_product[0]['return_code']); ?>";
        });
