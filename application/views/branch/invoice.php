                                        ...
                                        <table class="table w-full mb-4 table-clear">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="py-[0.9375rem] px-2.5  whitespace-nowrap sm:text-sm text-xs text-body-color text-right">  <?php if($formate[0]['seal'] == '1'){ ?> <img src="<?= base_url() ?>uploads/users/<?= $u['0']['sealimage'] ?>" width="100px" alt="" style="margin-top:-25px; height:100px; "> <?php } ?>
                                                    <?php if($formate[0]['sign'] == '1'){ ?><img src="<?= base_url() ?>uploads/users/<?= $u['0']['signimage'] ?>" width="100px" alt="" style="margin-top:-32px ; height:100px;"><?php } ?></td>
...
$payment = $this->CommonModal->getRowByIdOrderByLimit('payment', 'invoice_no',  $invoice['0']['invoice_no'],'branch_id',$u['0']['id'],'id','DESC','1');

$paymentsum = $this->CommonModal->getRowByIdSum('payment', 'invoice_no', $invoice['0']['invoice_no'],'branch_id',$u['0']['id'],'paid'); 

$customer = $this->CommonModal->getRowByMultitpleId('customer', 'id', $invoice['0']['customer_name'],'user_id',$u['0']['user_id']);
if (!empty($customer)) {
    foreach ($customer as $cus) {
        // Invoice Download URL
        $invoiceUrl = base_url('Admin/download_invoice/' . encryptId($u['0']['id']) . '/' . $invoice['0']['invoice_no']);
...
?>
    ...
    <script>
        document.getElementById('saveButton').addEventListener('click', function() {
            ...
            window.location.href = "<?php echo base_url('Branch_Dashboard/invoice/' . encryptId($u['0']['id'])); ?>";
        });
        document.getElementById('invoicen').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Branch_Dashboard/tax_invoice/' . encryptId($u['0']['id']) . '/' . $invoice[0]['invoice_no']); ?>";
        });
        ...
        document.getElementById('invoicen1').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Branch_Dashboard/normal_invoice/' . encryptId($u['0']['id']) . '/' . $invoice[0]['invoice_no']); ?>";
        });
    </script>
    <script>
