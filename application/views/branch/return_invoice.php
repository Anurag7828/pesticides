				...
				<div class="row">
					...
					<div class="xl:w-6/4 lg:w-4/3">
						<div class="card flex flex-col max-sm:mb-[30px] profile-card">
							...
							<form class="profile-form" action="<?= base_url('Branch_Dashboard/return_invoice?user_id=' . $user[0]['id'] . '&invoice_no=' . $invoice[0]['invoice_no'])  ?>" method="post" enctype="multipart/form-data">
											...
											<div class="row">
												<div class="xl:w-6/4 lg:w-4/3">
													<div class="card flex flex-col max-sm:mb-[30px] profile-card">
														...
														<div class="sm:p-10 sm:pb-2.5 p-[25px] pb-0">
														    	<?php  $p_produc = $this->CommonModal->getRowByMultitpleId('invoice', 'invoice_no', $invoice[0]['invoice_no'],'branch_id',$user[0]['id'],'user_id',$user[0]['user_id']); ?>
    ...
    <input type="hidden" name="unit[]" id="unit" class="form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full  " data-index="${rowIndex}" placeholder="Unit " required value="<?= $p_info['unit']?> " >
																	...
																	<div class="sm:w-1/6 w-full mb-[30px]">
														 			...
														 			$ava=	$p_info['quantity'] -	$p_info['return_quantity'];
														 				?>
														 					<input type="hidden" name="ava_quantity[]" class="form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full" id="availablee<?= $p_info['id']?>"  required readonly value="<?= $ava ?> ">
														 				<input type="text" name="quantity[]" class="form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500 outline-none w-full" placeholder="Quantity" required oninput="calculateTotalPrice(this.closest('.row'))" value="<?= $ava ?> " >
														 				                                                     <br>
            <?php if($p_info['return_quantity']) { ?>
                                                               <a href="<?= base_url('admin_Dashboard/view_return_invoice/'.encryptId($user['0']['id'])) ?>"><span class="text-xs py-[5px] px-3 rounded leading-[1.5] inline-block text-danger bg-danger-light"><?= $p_info['return_quantity'] ?> Return Product</span></a>
                                                                                  <?php } else { ?>
                                                                                  <?= "" ?>
                                                                                  <?php } ?>
																	</div>
                                                        ...
                                                        <option value="<?= $account_info['id']?>"><?= $account_info['bank_name']?></option>
                                                        <?php } ?>
