                ...
                <div class="row">
                                ...
                                <div class="mt-6 lg:w-1/4 md:w-1/2 w-full detail">
                                        <div class="2xl:w-7/12 w-full row items-center">
											<div class="sm:w-3/4"> 
												...
												<?php } else {?>
                                                 <div class="text-body-color sm:text-sm text-xs break-words"><strong style="font-size: 36px;line-height: 47px;"> <?= $u['0']['shop'] ?></strong></div>
                                                 <?php } ?>
									...
									<div class="mt-6 lg:w-1/3 md:w-1/2 w-full detail">
										...
										<div class="text-body-color sm:text-sm text-xs break-words"><strong>Shop Name.:</strong> <?= $u['0']['shop'] ?></div>
                                                <div class="text-body-color sm:text-sm text-xs"><strong>Contact No.:</strong> <?= $u['0']['contact'] ?></div>
                                                                                                             <div class="text-body-color sm:text-sm text-xs"><strong>Address:</strong> <?= $u['0']['address']?> <?= $u['0']['city']?> <br><?= $u['0']['district']?> ,<?= $u['0']['state']?> <?= $u['0']['pincode']?></div>
                                                <div class="text-body-color sm:text-sm text-xs"><strong>Email Id:</strong> <?= $u['0']['email'] ?></div>
                                                <div class="text-body-color sm:text-sm text-xs"><strong>GST NO.:</strong> <?= $u['0']['gst_no'] ?></div>
                                                <?php if($u['0']['lic_no'] ){ ?>
                                                <div class="text-body-color sm:text-sm text-xs"><strong>LIC No.:</strong> <?= $u['0']['lic_no'] ?></div>
                                                <?php } if($u['0']['cin_no']){?>
                                                 <div class="text-body-color sm:text-sm text-xs"><strong>CIN No.:</strong> <?= $u['0']['cin_no'] ?></div>
                                            <?php } ?>
                    ...
                    ?>
                                ...
                                <div class="overflow-x-auto table-scroll">
                                    <table class="table w-full mb-4 table-border">
                                        ...
                                        <tbody>
                                        <?php
                            $invoice_no = $this->CommonModal->getRowByMultitpleId('return_invoice', 'return_invoice_no', $invoice['0']['return_invoice_no'],'branch_id',$user['0']['id'],'user_id',$user['0']['user_id']);
                            $i=0;
                    ...
                    ?>
                                            <tr>
                                                ...
                                                <td class="py-[0.9375rem] px-2.5 border-b border-[#E6E6E6] dark:border-[#ffffff1a] whitespace-nowrap sm:text-sm text-xs text-body-color left"><?= $product[0]['packing']?> <?= $product[0]['unit']?></td>
                                ...
                                <div class="row">
                                   <div class="lg:w-1/2 sm:w-5/12 w-full ml-auto">
                                    ...
                                    <div class="lg:w-1/3 sm:w-5/12 w-full ml-auto">
                                        <table class="table w-full mb-4 table-clear">
                                        <?php
                             $payment = $this->CommonModal->getRowByIdOrderByLimit('return_invoice_payment', 'return_invoice_no',  $invoice['0']['return_invoice_no'],'branch_id',$user['0']['id'],'id','DESC','1');
                                                          $paymentsum = $this->CommonModal->getRowByIdSum('return_invoice_payment', 'return_invoice_no', $invoice['0']['return_invoice_no'],'branch_id',$user['0']['id'],'paid'); ?>
                                            <tbody>
...
if (!empty($customer)) {  
    foreach ($customer as $cus) {
        ...
        $invoiceUrl = base_url('Branch_Dashboard/print_return_invoice/' . encryptId($user['0']['id']) . '/' . $invoice['0']['invoice_no']);
?>
 <script>
...
document.getElementById('saveButton').addEventListener('click', function () {
    ...
    // Redirect to the desired page after the aler
    window.location.href = "<?php echo base_url('Branch_Dashboard/view_return_invoice/' . encryptId($user['0']['id'])); ?>";
});
...
document.getElementById('invoicen').addEventListener('click', function() {
            ...
            window.location.href = "<?= base_url('Branch_Dashboard/print_return_invoice_tax/' . encryptId($user[0]['id']) . '/' .$invoice[0]['return_invoice_no']); ?>";
        });
        ...
        document.getElementById('invoicen1').addEventListener('click', function() {
            ...
            // Redirect to the desired page after the alert
            window.location.href = "<?= base_url('Branch_Dashboard/print_return_invoice_normal/' . encryptId($user[0]['id']) . '/' .$invoice[0]['return_invoice_no']); ?>";
        });
